...
def main(predictions_dir, output_seg_derivative_path, margin=None, use_prob_map=False, foregrnd_threshold=0.5, backgrnd_threshold=0.5):
    ...
    patient_match_name = json.load(open(os.path.join(os.path.dirname(predictions_dir), 'name_correspondance.json')))
    failed = []
...
if __name__ == "__main__":
    ...
    predictions_dir = os.path.join(main_dir, "derivatives/3_all_images_grouped_nnunet_struct/predictions")
    output_seg_derivative_path = os.path.join(main_dir, 'derivatives/4_liver_masks_corrected/liver_masks_no_margin_original_threshold')
