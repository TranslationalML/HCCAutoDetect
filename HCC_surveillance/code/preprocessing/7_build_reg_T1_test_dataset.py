...
def main(output_derivative_path, data_dir, tumor_seg_dir, liver_seg_dir, total_seg_dir, margin, save_as_4D, subtract):
    ...
    for patient, sub_id in participants[['Patient', 'Sub-ID']].values:
        ...
        if sub_id not in df['ID'].values:
            removed_patient[sub_id] = "Patient excluded"
            continue

        if not any([x in sub_id for x in [f'sub-{str(i).zfill(3)}' for i in range(97, 119 + 1)]]) and not healthy_patient:
            removed_patient[sub_id] = "Excluded since it is not a test patient"
            continue
        ...
        if not healthy_patient:
            ...
            sub_df_conc = pd.concat([sub_df_ID, sub_df_art, sub_df_ven_washout, sub_df_ven_capsule,
                                    sub_df_del_washout, sub_df_del_capsule, sub_df_LIRADS, sub_df_HCC, sub_df_loc, sub_df_size], axis=1)
        ...
        if not healthy_patient:        
            ...
            all_TTC_lesion_lab = []
        if len(all_TTC_lesion_lab) != 0 or healthy_patient:
            if not healthy_patient:
                ...
                if sub_id in ['sub-117']:
                    pass
                else:
                    with open(patient_dyn_files_to_select + '/dyn/phases_to_keep.json', 'r') as f:
                ...
                is_TTC = all(any(x in file for file in img_list) for x in ['TTC_1','TTC_2', 'TTC_3'])
                if is_TTC:
                    TTC_with_most_tumors = '3'
            ...
            if is_TTC:
                img_list = [file for file in img_list if any([x in file for x in ['native', 'TTC_' + TTC_with_most_tumors, 'venous', 'delayed']])]
            else:
            ...
            if not healthy_patient:
                ...
                multilab_mask = sitk.Image(liver_mask.GetSize(), sitk.sitkUInt8)
                multilab_mask.CopyInformation(liver_mask)
                final_mask = sitk.Image(liver_mask.GetSize(), sitk.sitkUInt8)
                final_mask.CopyInformation(final_mask)
        ...
        final_mask_roi.CopyInformation(liver_mask_roi)
        ...
        rb_mask.SetOrigin(liver_mask_roi.GetOrigin())
        rb_mask.SetSpacing(liver_mask_roi.GetSpacing())
        rb_mask.SetDirection(liver_mask_roi.GetDirection())
    ...
    mean_lesions_per_patient = np.mean([value for key, value in lesion_per_patient_dict.items()])
    std_lesions_per_patient = np.std([value for key, value in lesion_per_patient_dict.items()])
...
if __name__ == "__main__":
    ...
    output_derivative_path = os.path.join(main_dir, 'derivatives/10_test_T1_dataset')
