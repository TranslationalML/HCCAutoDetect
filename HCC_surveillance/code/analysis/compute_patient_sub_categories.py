...
def main(data_path):
    ...
    for patient, patient_ID in participants[['Patient', 'Sub-ID']].values:
        if patient_ID + '.nii.gz' in os.listdir(images) == False:
            continue
        ...
        if len(art_contrasts_in_patient_dict) != len(labels):
            print()
    ...
    # patient selection with only lesions respcting contrast
    ...
    with open(os.path.join(data_path, "patient_selection/patient_wise_lesion_diameter.json"), 'w') as f:
        json.dump({'All lesions': lesion_size,
    ...
    lesions_select_all_no_nan = {k_patient: v_patient for k_patient, v_patient in lesion_size.items() if len(v_patient) > 0}
    ...
    lesion_selection_lesions_per_contrast_category = {k: len(v) for k, v in {'contrast <= 1': lesions_art_contrast_hypo_no_nan,
    ...
    lesion_selection_patients_per_diameter_category = {k: len(v) for k, v in {
                                                            'All lesions': [k_patient + '_' + k_lesion for k_patient, v_patient in lesions_select_all_no_nan.items() for k_lesion, v_lesion in v_patient.items()],
    ...
    with open(os.path.join(data_path, "patient_selection/lesion_wise_lesion_diameter.json"), 'w') as f:
        json.dump({'All lesions': lesion_size,
                    'lesion_diameter_less_70mm': lesions_selec_lesions_less_70mm,
    ...
    with open(os.path.join(data_path, "patient_selection/lesions_information.json"), 'w') as f:
        json.dump({'patient_wise_selection:_lesions_per_contrast_category': lesions_per_contrast_category,
                   ...
                   'mean number of lesions per patient': mean_nbr_lesions_per_patient,
                    ...
                    'mean HCC size': np.mean([v_lesion for v_patient in patient_HCC_size.values() for v_lesion in v_patient.values()]),
                    'std HCC size': np.std([v_lesion for v_patient in patient_HCC_size.values() for v_lesion in v_patient.values()]),
                   }, f, indent=4)
...
if __name__ == "__main__":
    cwd = os.getcwd()
    # data_path = os.path.dirname(os.path.dirname(cwd)) + "/derivatives/10_test_T1_dataset"
    # main(data_path, exclude_patients)

    data_path = os.path.dirname(os.path.dirname(cwd)) + "/derivatives/10_T1_dataset"
    main(data_path)
