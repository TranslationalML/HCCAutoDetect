...
def main(data_to_predict, output_dir, data_set_ID):
    ...
    all_files = [glob.glob(os.path.join(data_to_predict, patient_id + '/dyn', "*.nii.gz"))
                   for patient, patient_id in participants[['Patient', 'Sub-ID']].values]
    all_files = [item for sublist in all_files for item in sublist]
    all_files = [file for file in all_files if 'venous' in file]
    ...
    source = os.getcwd() + '/work_dir/nnUNet_raw_data_base/Dataset007_5_2_T1_registration_groupwise/imagesTr/'
    ...
    json_convert_names_dict = os.path.join(main_dir, 'training_evaluation/liver_segmentation/nnunet/work_dir/nnUNet_raw_data_base/Dataset007_5_2_T1_registration_groupwise/datalist.json')
...
if __name__ == "__main__":
    ...
    main_dir = os.path.dirname(os.path.dirname(os.path.dirname(cwd)))
    data_set_ID = 7
    data_to_predict = main_dir + '/HCC_pre_ablation/derivatives/5_T1_registration_groupwise/'
    output_dir = main_dir + '/HCC_pre_ablation/derivatives/5_liver_masks_registered/'
