...
from preprocessing.utils import getDynList
...
def main(input_patients_path, output_derivative_path, config):
    ...
    for patient, patient_id in participants[['Patient', 'Sub-ID']].values:
        ...
        if not dyn_list:
            print(f'No dynamic images found for {patient_id}. Skipping registration.')
            continue


        population = sorted(dyn_list)
...
if __name__ == "__main__":
    ...
    input_path = os.path.join(main_dir, 'derivatives/4_T1_registration_pairwise')
    output_path = os.path.join(main_dir, 'derivatives/5_T1_registration_groupwise')
