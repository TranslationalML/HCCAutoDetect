...
from monai.transforms.croppad.batch import PadListDataCollate, list_data_collate
...
__all__ = ["CustomTestTimeAugmentation"]
...
class CustomTestTimeAugmentation:
    """
    ...
    def __init__(
        ...
        sliding_window_mode: str = "gaussian",
        sliding_window_sigma_scale: float = 0.25,
    ) -> None:
        ...
        self.overlap = sliding_window_overlap
        self.mode = sliding_window_mode
        self.sigma_scale = sliding_window_sigma_scale

        self._pred_key = CommonKeys.PRED
        self.inverter = Invertd(
            keys=self._pred_key,
            transform=transform,
            orig_keys=self.label_key,
    ...
    def __call__(
        ...
        dl = DataLoader(ds, num_workers=self.num_workers, batch_size=self.batch_size, collate_fn=list_data_collate)

        outs: list = []
        with torch.no_grad():
            for b in tqdm(dl) if has_tqdm and self.progress else dl:
                # do model forward pass
                outputs = sliding_window_inference(b[self.image_key].to(self.device),
                                                   self.patch_size,
                                                   sw_batch_size=self.batch_size,
                                                   predictor=self.inferrer_fn,
                                                   overlap=self.overlap,
                                                   mode=self.mode,
                                                   sigma_scale=self.sigma_scale
                                                   )
                b[self._pred_key] = outputs
                inv_b = [self.inverter(PadListDataCollate.inverse(i))[self._pred_key] for i in decollate_batch(b)]
                outs.extend(inv_b)
